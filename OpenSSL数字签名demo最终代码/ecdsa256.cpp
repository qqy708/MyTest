...
#include "ecdsa.h"
//密钥对生成
int Ecdsa::newPair256(const unsigned char* priv_b)
{
	EC_KEY* key;
	BIGNUM* priv;
	BN_CTX* ctx;
	...
	this->setPriv256(priv_b);

	/** create a ec keypair **/
	key = EC_KEY_new_by_curve_name(NID_brainpoolP256r1);

	cout << "\t\t椭圆曲线NID_brainpoolP256r1生成的密钥对为: " << key << endl;
	cout << endl;
...
//此函数将文件内容进行SHA256哈希并且展示哈希值，文件内容为const char* 类型的数据
int Ecdsa::sign256(unsigned char** der, unsigned int* dlen, const char* message, unsigned int len)
{
	ECDSA_SIG* signature;//结构体ECDSA_SIG是头文件ec.h里自带的定义的结构体，这里是这个结构体的指针
	unsigned int der_len;
	unsigned char* der_copy;

	string dst = sha256(message, len);
	cout << "\t\t文件的SHA256哈希值为:\t" << dst << endl << endl;
...
//验证签名的函数，里面需要的参数和sign256非常相似，而且都是需要传一个数据类型为const char*的文件内容和这个文件内容的字符串长度
int Ecdsa::verify256(const unsigned char* der, unsigned int der_len, const char* message, unsigned int len)
{
	...
	string dst = sha256(message, len);
	cout << "\t\t再次查看文件的SHA256哈希值为:\t" << dst << endl << endl;
	const unsigned char* dst1 = (const unsigned char*)dst.c_str();
...
int Ecdsa::print256()
{
	cout << "\t\t密钥为: " << this->priv_b_length << "字节) #:" << endl;
	cout << endl;
	...
	cout << endl << endl;
	cout << dec << "\t\t未压缩的公钥： (" << this->pub_uncom_length << "字节) #:" << endl;
	cout << endl;
	...
	cout << endl << endl;
	cout << dec << "\t\t压缩后的公钥： (" << this->pub_com_length << "字节) #:" << endl;
	cout << endl;
...
//生成私钥的函数
int Ecdsa::setPriv256(const unsigned char* priv_b)
{
	this->priv_b = priv_b;
...
int Ecdsa::setPub256()
{
	...
	this->p_key = EC_KEY_new_by_curve_name(NID_brainpoolP256r1);
...
//SHA256函数进行哈希与填充
string Ecdsa::sha256(const char* message, unsigned int len) {

	unsigned char digest1[SHA256_DIGEST_LENGTH];//空的char数组，用来装SHA256的哈希值
	SHA256_CTX ctx;//openSSL里定义的一个SHA256结构体
	SHA256_Init(&ctx);//此函数初始化这个结构体
	SHA256_Update(&ctx, message, len);//此函数是进行哈希运算的函数，可以被连续调用
	SHA256_Final(digest1, &ctx);//将最终的哈希结果存储在digest1这个数组里，此时的哈希值是一堆乱码，我们必须转化成16进制格式
	stringstream ss;
