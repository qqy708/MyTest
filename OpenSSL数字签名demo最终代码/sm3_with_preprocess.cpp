	...
	unsigned char* z_digest)
{
	...
	EVP_DigestInit_ex(md_ctx, md, NULL);//设置摘要上下文ctx以使用ENGINE impl中的摘要类型。类型通常由EVP_sha1（）等函数提供。如果impl为NULL，则使用摘要类型的默认实现。
	EVP_DigestUpdate(md_ctx, entl, sizeof(entl));//哈希数据到md_ctx里面去，此函数可以被循环调用
	...
	EVP_DigestFinal_ex(md_ctx, z_digest, NULL);//从md_ctx里面提取出摘要值然后存到z_digest里面，最终的摘要将会存储到z_digest里
	EVP_MD_CTX_free(md_ctx);//释放md_ctx 和它占用的空间.
	return 0;
}
